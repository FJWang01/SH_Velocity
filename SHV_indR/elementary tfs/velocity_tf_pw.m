...
function [v_x, v_y, v_z] = velocity_tf_pw(R, theta_pw, phi_pw, k, rho, c)
% p = velocity_tf_pw(R, theta_pw, phi_pw, k, rho, c)
% 
% This function calculates the velocity at point receiver R due to plane
...
%
% Outputs:
% [v_x, v_y, v_z] - velocity vector at point receiver R due to plane wave 
%   with incidence direction (theta_pw, phi_pw)
%   size(v_x) = [size(R, 1), numel(theta_pw), numel(k)]

%% Check the dimensions of inputs
if ~isequal(size(R, 2), 3)
    error('@@ velocity_tf_pw: R must have three columns');
else
    % do nothing
end

if ~isequal(size(theta_pw), size(phi_pw))
    error('@@ velocity_tf_pw: theta_pw and phi_pw must be of the same size');
else
    % do nothing
end
...
%% Calculate velocity at point receiver R
...
scaling_factor = 1/rho/c;

v = scaling_factor * exp(1i * k_3D .* dot_prod_3D);
